...
const posts = [
  {
    ...
    facts: [["VERSION", "1.8.1"], ["PLATFORM", "Obsidian"], ["PRINCIPLE", "Configurable"]],
    blocks: [
      ...
      { type: "h2", text: "1.8.1：从个人插件变成通用学习工具" },
      { type: "p", id: "universal-goal", text: "早期版本的核心能力可以用于普通 Vault，但自动分类仍带有我的课程结构。1.8.0 把这些写死的规则移入设置层；1.8.1 又把原来的文本规则升级为可视化类别管理器，每个领域都能直接编辑和删除，并有明确的“添加新学习领域”按钮。" },
      { type: "list", id: "universal-options", items: ["在可视化管理器中新增、删除或重命名学习领域，并设置关键词、图标和颜色", "在学习与归类下拉框中直接选择“新建或管理学习领域”", "以同样方式自由定义资料类型及其自动识别关键词", "把插件内部字段映射到已有的中文、英文或自定义 Frontmatter", "更换新建笔记模板路径和没有核心日记配置时的备用目录", "自定义任务日期标记，例如 deadline:: 或 截止::", "调整困难、一般和熟练三种反馈的最短间隔、倍率与掌握度增量", "通过 JSON 在多个 Vault 之间迁移通用配置，不导出笔记正文和按文件保存的个人样式"] },
      { type: "p", id: "universal-example", text: "例如医学用户可以建立“解剖学、药理学、临床案例”，科研用户可以建立“论文阅读、实验记录、研究想法”；英文 Vault 也可以把字段映射为 study_status、mastery 和 next_review。旧版中文属性仍会被兼容读取，所以现有用户升级后不需要重建知识库。" },
