...
const posts = [
  {
    ...
    facts: [["VERSION", "1.8.0"], ["PLATFORM", "Obsidian"], ["PRINCIPLE", "Configurable"]],
    blocks: [
      ...
      { type: "h2", text: "1.8.0：从个人插件变成通用学习工具" },
      { type: "p", id: "universal-goal", text: "早期版本的核心能力可以用于普通 Vault，但自动分类仍带有我的课程结构。1.8.0 把这些写死的规则全部移入设置层：插件保留一套可直接使用的示例，同时允许每个人根据自己的专业、语言和笔记属性重建分类体系。" },
      { type: "list", id: "universal-options", items: ["自由新增、删除或重命名学习领域，并设置关键词、图标和颜色", "自由定义资料类型及其自动识别关键词", "把插件内部字段映射到已有的中文、英文或自定义 Frontmatter", "更换新建笔记模板路径和没有核心日记配置时的备用目录", "自定义任务日期标记，例如 deadline:: 或 截止::", "调整困难、一般和熟练三种反馈的最短间隔、倍率与掌握度增量", "通过 JSON 在多个 Vault 之间迁移通用配置，不导出笔记正文和按文件保存的个人样式"] },
